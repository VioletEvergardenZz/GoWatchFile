...
### 1.3 演进总目标（12~18 个月）
将系统从“单机可用工具”演进为“多 Agent 文件运维平台”：
1. **可运营**：稳定、可观测、可恢复。
2. **可治理**：多租户、多团队、审计、权限。
...
- **决策 3：短期双主线并行**。  
  一条线稳上传可靠性与控制面基础，一条线做 AIOps 数据质量与分析闭环。
...
## Sprint 7-8（第 7~8 周）：多 Agent MVP
- Agent 注册/心跳 API。
- Agent 分组 + 标签管理。
- 控制台新增 Agent 列表页与状态页。
- 离线告警与升级通知策略。
...
## Sprint 9-10（第 9~10 周）：任务模型与路由基础
- 引入统一 Task 模型与状态机。
