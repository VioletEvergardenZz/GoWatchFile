...
func (h *handler) kbRecommendations(w http.ResponseWriter, r *http.Request) {
	...
	if query == "" {
		query = strings.TrimSpace(r.URL.Query().Get("rule"))
	}
	if query == "" {
		query = strings.TrimSpace(r.URL.Query().Get("message"))
	}
	if query == "" {
		query = strings.TrimSpace(r.URL.Query().Get("alertId"))
	}
	limit := parsePositiveInt(r.URL.Query().Get("limit"), 3)
	if query == "" {
		writeJSON(w, http.StatusOK, map[string]any{"ok": true, "items": []kb.Article{}})
		return
	}
