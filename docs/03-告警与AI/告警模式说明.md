...
```powershell
cd go-watch-file
powershell -ExecutionPolicy Bypass -File scripts/ops/alert-rules-compose.ps1 -TemplateFile ./deploy/alert/gwf-alert-rules-layered-template.json -Scenarios "database,upload,ai,control" -OutputFile ../reports/alert-rules-composed.json -Apply -BaseUrl http://localhost:8082 -Token $env:API_AUTH_TOKEN
```
