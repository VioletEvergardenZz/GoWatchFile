...
### 5.3 可解释字段（`decision.explain`）
- `decisionKind`：决策来源，`rule_match`（规则匹配）或 `escalation`（异常升级）。
- `notify`：该条决策是否具备通知资格（`true` 发送，`false` 仅记录）。
- `suppressionEnabled`：运行态抑制开关是否启用（对应 `alert_suppress_enabled`）。
- `suppressWindow`：本次决策使用的抑制窗口展示值（如 `5分钟`）。
- `suppressedBy`：当状态为 `suppressed` 时的抑制来源：
  - `rule_window`：规则抑制窗口。
  - `escalation_window`：升级抑制窗口。
- `escalationThreshold` / `escalationWindow` / `escalationCount`：仅升级决策返回，用于说明“在多大窗口、达到多少阈值、当前计数是多少”。
...
- 规则抑制触发时，`decision.explain.suppressedBy=rule_window`。
...
- 升级被窗口抑制时，`decision.explain.suppressedBy=escalation_window`。
...
- **告警列表**：展示最近决策、抑制原因和结构化解释字段（来源/通知/抑制/升级计数），支持分页。
...
`GET /api/alerts` 的 `data.decisions[]` 关键字段示例：
```json
{
  "id": "42",
  "status": "suppressed",
