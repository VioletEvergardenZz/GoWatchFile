...
### 环境变量覆盖范围
- 仅会覆盖 OSS / 通知 / API 安全 / AI / 上传静态策略相关字段（`OSS_*`、`DINGTALK_*`、`EMAIL_*`、`ROBOT_KEY`、`API_*`、`AI_*`、`UPLOAD_QUEUE_PERSIST_*`、`UPLOAD_QUEUE_SATURATION_THRESHOLD`、`UPLOAD_QUEUE_CIRCUIT_BREAKER_ENABLED`、`UPLOAD_RETRY_MAX_ATTEMPTS`、`UPLOAD_ETAG_VERIFY_ENABLED`、`UPLOAD_RESUMABLE_*`）。
...
```
若要直接发布到后端，增加 `-Apply -BaseUrl http://localhost:8082 -Token $env:API_AUTH_TOKEN`。
...
```
若后端未启用鉴权，`ai-replay.ps1` / `ai-baseline.ps1` / `stage-prime.ps1` / `control-replay.ps1` / `control-agent-check.ps1` / `alert-rules-compose.ps1` / `stage-recap.ps1` 可直接不传 `-Token`。
