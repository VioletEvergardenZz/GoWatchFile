...
### 环境变量覆盖范围
- 仅会覆盖 OSS / 通知 / API 安全 / AI / 上传静态策略相关字段（`OSS_*`、`DINGTALK_*`、`EMAIL_*`、`ROBOT_KEY`、`API_*`、`AI_*`、`UPLOAD_QUEUE_PERSIST_*`、`UPLOAD_QUEUE_SATURATION_THRESHOLD`、`UPLOAD_QUEUE_CIRCUIT_BREAKER_ENABLED`、`UPLOAD_RETRY_MAX_ATTEMPTS`）。
...
- `upload_queue_persist_enabled` / `upload_queue_persist_file` / `upload_queue_saturation_threshold` / `upload_queue_circuit_breaker_enabled` / `upload_retry_max_attempts` 不在 `/api/config` 更新范围内，需改 `config.yaml` 或 `.env` 后重启。
...
```
可先复制 `docs/03-告警与AI/AI回放路径样本.txt`，再按环境修改 `docs/03-告警与AI/AI回放路径清单.txt`。
...
```
若后端未启用鉴权，`ai-replay.ps1` / `stage-prime.ps1` / `control-replay.ps1` / `stage-recap.ps1` 可直接不传 `-Token`。
