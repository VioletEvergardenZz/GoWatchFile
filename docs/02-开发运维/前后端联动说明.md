...
sequenceDiagram
  ...
  API-->>UI: { ok, analysis, meta{usedLines,truncated,elapsedMs,retries,degraded,errorClass} }
```

说明：需后端开启 `ai_enabled` 并配置 `AI_*`；若 AI 不可用会返回降级结果（`meta.degraded=true`）。
...
- 运行态未就绪时后端返回降级结构（`200`），用于避免前端直接报 `500`
- 返回：`DashboardData`
...
- 返回 `meta`：`usedLines/truncated/elapsedMs/retries/degraded/errorClass`。
- 路径范围：`path` 支持 `watch_dir` 下文件，也支持 `alert_log_paths` 中配置的日志路径（用于后端报错日志分析）。

**7) 健康检查**
- `GET /api/health`
- 返回：`{ queue, workers, inFlight, queueFullTotal, queueShedTotal, retryTotal, uploadFailureTotal, failureReasons, persistQueue }`
...
- 查询参数：`mode=lite|mode=light` / `limit` / `includeEnv=true`
- 返回：`SystemDashboard`
- 说明：需开启 `systemResourceEnabled`，否则返回 403；默认隐藏进程环境变量。

**12) Prometheus 指标**
- `GET /metrics`
- 返回：Prometheus 文本暴露格式（上传、队列、AI、知识库指标）。

---

## 5. 注意事项
- `/api/config` 更新运行态配置，并在可写时持久化到 `config.runtime.yaml`；OSS 与通知配置需改 `.env` 并重启。
- `upload_queue_persist_*`、`upload_queue_saturation_threshold`、`upload_queue_circuit_breaker_enabled`、`upload_retry_max_attempts` 为静态配置，不支持通过 `/api/config` 在线切换，修改后需重启。
...
- `/api/health` 仅用于运行健康快照；长期趋势与告警建议使用 `/metrics`。
- 告警概览统计窗口为最近 24 小时。
- 前端的 `concurrency` 字段是字符串（例如 `workers=3 / queue=100`），保存时解析成数值。
- 如需完整字段解释与格式约定，参考 `docs/99-架构附录/状态类型可视化.md`。
- 队列持久化行为与运维命令参考 `docs/02-开发运维/队列持久化运行手册.md`。
