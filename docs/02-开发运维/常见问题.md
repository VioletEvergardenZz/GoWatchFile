...
## 26. 执行前端门禁时报 `node: command not found` 或 `npm: command not found` 怎么办？
- 当前仓库前端门禁命令是：`cd console-frontend && npm run build`。
- macOS + nvm 场景下，若新开终端命令缺失，先执行：
  - `source /etc/profile`
  - `node -v && npm -v`
- 版本确认正常后再执行前端门禁命令。
- 若仍失败，优先检查 `~/.nvm` 初始化脚本是否已写入 `/etc/profile`（或你实际使用的 shell 启动文件）。
