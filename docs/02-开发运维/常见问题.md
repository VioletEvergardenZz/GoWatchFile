...
- 新版控制台在点击“立即启用”失败时，会在页面内直接显示失败原因（如鉴权失败、后端不可达），可直接按提示排查。
...
## 21. 我暂时不想用 API Token，怎么配置？
- 当前版本支持“可选鉴权”。
- 满足以下任一条件即可关闭管理接口 token 校验：
  - `API_AUTH_TOKEN` 为空；
  - `API_AUTH_TOKEN` 仍为占位符 `${API_AUTH_TOKEN}`；
  - 显式设置 `API_AUTH_DISABLED=true`。
- `/api/health` 始终允许匿名访问。
- 控制台顶部的 `API 鉴权 Token（可选）` 属于高级项，仅在后端开启鉴权时才需要设置；未开启时可忽略。
...
- 改完后必须重启后端进程。
- 如果你不需要鉴权（`API_AUTH_TOKEN` 为空或 `API_AUTH_DISABLED=true`），新版后端在 `API_CORS_ORIGINS` 为空时会默认放行所有来源。
- 若你开启了鉴权但未配置 `API_CORS_ORIGINS`，后端会默认放行本机回环来源（`localhost/127.0.0.1/::1`）和同主机来源。
