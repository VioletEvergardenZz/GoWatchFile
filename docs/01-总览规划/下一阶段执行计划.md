...
> 目标：在不扩张范围的前提下，完成“文件入云稳定 + 告警决策可用 + AI 分析可降级 + 知识复用可追溯”的主线收口。

> 平台定位（当前口径）：面向 SRE/运维的运维事件闭环平台，主线聚焦“文件入云 + 告警决策 + AI 分析 + 知识库复用”。
...
- 路由编排 DSL 与复杂工作流
- 复杂分片编排与合并校验（本阶段仅补齐 OSS 断点续传基础能力）
...
## 21. 2026-02-18 进展更新（虚拟机 API 500 排障与接口访问策略收口）
- 已完成：
  - 后端移除 API Token 鉴权逻辑，管理接口默认匿名可访问。
...
- 文档更新：
  ...
  - `docs/02-开发运维/开发指南.md`
- 当前状态：
  - 上限/退避 + ETag 校验能力已稳定运行。
  - 与断点续传能力合并后，“重试策略完善（上限/退避）、断点续传/etag 校验”已闭环完成（见第 30 条）。

## 30. 2026-02-19 进展更新（上传可靠性断点续传补齐）
- 已完成：
  - 上传链路新增断点续传静态配置（需重启生效）：
    - `upload_resumable_enabled`
    - `upload_resumable_part_size`
    - `upload_resumable_routines`
    - `upload_resumable_threshold`
    - `upload_resumable_checkpoint_dir`
  - 环境变量覆盖补齐：
    - `UPLOAD_RESUMABLE_ENABLED`
    - `UPLOAD_RESUMABLE_PART_SIZE`
    - `UPLOAD_RESUMABLE_ROUTINES`
    - `UPLOAD_RESUMABLE_THRESHOLD`
    - `UPLOAD_RESUMABLE_CHECKPOINT_DIR`
  - OSS 上传链路补齐断点续传分支（基于 SDK `UploadFile` + checkpoint）：
    - 达到阈值后切换 multipart + checkpoint 上传
    - 补齐 part size / routines / threshold / checkpoint 解析与边界处理
    - 断点续传链路下 ETag 不做本地 MD5 严格比对（multipart ETag 非纯文件 MD5）
  - 新增/补充单元测试：
    - `go-watch-file/internal/oss/client_test.go`
    - `go-watch-file/internal/config/config_test.go`
- 文档更新：
...
## 31. 2026-02-19 进展更新（控制面状态机口径收口）
- 已完成：
  - 统一控制面任务状态机规则（超时/重试/取消）：
    - `running` 超时后，若 `retryCount < maxRetries` 则自动重试并回到 `pending`
    - 达到重试上限后，`running` 超时进入 `timeout` 终态
    - `cancel/retry` 的可操作状态判断改为统一函数，避免分散逻辑口径漂移
  - 新增状态机异常回归覆盖：
    - `assigned` 超时回收重排
    - `running` 超时自动重试
    - `running` 超时且无重试预算时终态收口
    - 非法重试（`pending`）拒绝
    - 终态取消拒绝
- 代码：
  - `go-watch-file/internal/api/control_task_state_machine.go`
  - `go-watch-file/internal/api/control_dispatch.go`
  - `go-watch-file/internal/api/control_handlers.go`
  - `go-watch-file/internal/api/control_task_state_machine_test.go`
- 文档更新：
  - `docs/04-路线图与计划/TODO.md`
- 验证结果：
  - `cd go-watch-file && go test ./internal/api -count=1` 通过

## 32. 2026-02-19 进展更新（控制面失败原因分布视图）
- 已完成：
  - 新增控制面失败原因分布接口：
    - `GET /api/control/tasks/failure-reasons`
    - 支持参数：`status=failed,timeout,canceled`、`type=`、`limit=`
  - 新增任务失败原因字段并接入状态流转：
    - `task complete(failed)` 记录失败原因（优先 `error`，其次 `message`）
    - `running timeout` 终态记录 `run_timeout`
    - `cancel` 记录 `manual_cancel`
    - `retry` 回到 `pending` 时清空失败原因
  - 控制台控制面页新增“失败原因分布（失败+超时）”TopN 可视化。
- 代码：
  - `go-watch-file/internal/api/control_failure_reasons.go`
  - `go-watch-file/internal/api/control_handlers.go`
  - `go-watch-file/internal/api/control_dispatch.go`
  - `go-watch-file/internal/api/control_store.go`
  - `go-watch-file/internal/api/control_failure_reasons_test.go`
  - `go-watch-file/internal/api/control_store_test.go`
  - `console-frontend/src/ControlConsole.tsx`
  - `console-frontend/src/ControlConsole.css`
  - `console-frontend/src/console/dashboardApi.ts`
  - `console-frontend/src/types.ts`
- 文档更新：
  - `docs/04-路线图与计划/TODO.md`
...
## 33. 2026-02-19 进展更新（Agent 在线状态判定文档与巡检手册）
- 已完成：
  - 新增控制面 Agent 在线状态判定与巡检手册：
    - `docs/02-开发运维/控制面Agent在线状态判定与巡检手册.md`
  - 新增 Agent 在线巡检脚本（统一在线口径 + `/metrics` 对账 + 门禁退出码）：
    - `go-watch-file/scripts/ops/control-agent-check.ps1`
    - 支持参数：`-OfflineAfterSec`、`-FailOnOffline`、`-MaxOfflineAgents`、`-OutputFile`、`-ReportFile`
    - 退出码：`0=通过`、`2=接口请求失败`、`4=离线数超阈值`
  - 同步更新文档入口与运维命令示例：
    - `go-watch-file/README.md`
    - `docs/02-开发运维/开发指南.md`
    - `docs/02-开发运维/README.md`
  - `TODO` 对应 P0 条目已标记完成：
    - `docs/04-路线图与计划/TODO.md`
- 验证结果：
  - `powershell -ExecutionPolicy Bypass -File go-watch-file/scripts/ops/control-agent-check.ps1 -BaseUrl http://127.0.0.1:1` 返回预期退出码 `2`（不可达场景自检）
  - `cd go-watch-file && go test ./... -count=1` 通过
  - `cd console-frontend && npm run build` 通过

## 34. 2026-02-19 进展更新（告警规则模板双层结构收口）
- 已完成：
  - 新增告警规则双层模板（默认规则 + 场景规则）：
    - `go-watch-file/deploy/alert/gwf-alert-rules-layered-template.json`
    - 结构：`defaultRules`（默认层）+ `scenarios[].rules`（场景层）
  - 新增模板组合脚本（可选直接发布到 `/api/alert-rules`）：
    - `go-watch-file/scripts/ops/alert-rules-compose.ps1`
    - 支持参数：`-Scenarios`、`-IncludeAllScenarios`、`-Apply`、`-BaseUrl`
    - 退出码：`0=成功`、`2=模板/校验失败`、`3=场景不存在`、`4=发布失败`
  - 新增专题手册：
    - `docs/03-告警与AI/告警规则双层模板手册.md`
  - 同步更新文档入口与操作说明：
    - `go-watch-file/README.md`
    - `docs/02-开发运维/开发指南.md`
    - `docs/03-告警与AI/README.md`
    - `docs/03-告警与AI/告警模式说明.md`
  - `TODO` 对应 P0 条目已标记完成：
    - `docs/04-路线图与计划/TODO.md`
- 验证结果：
  - `powershell -ExecutionPolicy Bypass -File go-watch-file/scripts/ops/alert-rules-compose.ps1 -TemplateFile go-watch-file/deploy/alert/gwf-alert-rules-layered-template.json -Scenarios "database,upload" -OutputFile reports/alert-rules-composed-selfcheck.json` 通过
  - `cd go-watch-file && go test ./... -count=1` 通过
  - `cd console-frontend && npm run build` 通过
