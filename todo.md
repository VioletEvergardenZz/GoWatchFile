...
- [x] 完成 `/metrics` 采集接入与阈值告警配置（队列/失败率/AI 降级率，含 Prometheus 规则模板与巡检阈值门禁）
- [x] 完成上传链路压测与故障注入报告（OSS 抖动/网络超时/队列饱和，新增 upload-recap 一键复盘与报告输出）
- [x] 完成 AI 回放样例集与降级结果验收（结构一致性 + 错误分类）
- [x] 完成知识库命中率/引用率阶段复盘与改进清单
- [x] 输出控制面 MVP 设计草案（范围、数据模型、API、验收标准）
- [x] 落地控制面首批接口（Agent register/heartbeat/list + Task create/list/get/cancel/retry）
- [x] 落地控制面 SQLite 持久化（agents/tasks 重启恢复）
...
- [x] 可靠性：临时文件过滤（常见临时后缀）。  
- [x] 可靠性：重试策略完善（上限/退避）、断点续传/etag 校验。  
- [x] 并发与背压：工作池 + 队列（内存）。
...
- [x] 设计：控制面 MVP 草案（`docs/01-总览规划/控制面MVP设计草案.md`）。  
- [x] 实现：控制面 API v1 `/api/control/agents*`、`/api/control/tasks*`。  
- [x] 实现：控制面持久化存储（SQLite）与启动恢复。  
...
- [-] 大文件策略：已完成断点续传（OSS multipart + checkpoint），待补分片后续动作编排与合并校验。  
