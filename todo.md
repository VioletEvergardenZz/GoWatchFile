...
> 目标：依照大纲模块，将 Java 堆转储链路演进为通用“文件监控 + 上传 + 路由/编排 + 可视化”平台。先稳固 Agent，再补齐控制面、路由编排、可视化与分析。
...
## 阶段 1：Agent 采集与上传传输
- [ ] 文件匹配：多后缀、忽略目录/文件、按子目录/标签分流前缀。  
- [ ] 可靠性：失败重试（上限/退避）、断点续传/etag 校验、临时文件过滤。  
- [ ] 并发与背压：队列饱和阈值/告警，限流/熔断，静默时间可调。  
- [ ] 指标与日志：Prometheus（事件速率、队列长度、上传耗时、错误码），结构化日志。  
- [ ] 配置体验：启动校验、错误提示，热更新/重新加载（可选）。  
...
## 阶段 5：可视化界面与告警
- [ ] Web 控制台 MVP：总览仪表（成功率/吞吐/滞留）、事件时间线、任务详情。  
- [ ] 失败/滞留列表：筛选、批量重试/下载、失败原因可视化。  
- [ ] 日志可视化：实时 tail、全文检索/关键词搜索，对接 ELK/Loki/Graylog 或内置轻量视图。  
...
## 数据与配置草案
- **Agent 配置**：watch_dir、file_ext（多值）、ignore 列表、静默时间、upload_workers、upload_queue_size、S3（bucket/ak/sk/endpoint/region/force_path_style/disable_ssl）、通知（企微/钉钉）、日志。  
- **表设计雏形**：  
